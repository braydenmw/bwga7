      ...
      >
        <header className="p-4 flex justify-between items-center border-b border-nexus-border flex-shrink-0">
          <div className="flex items-center gap-3">
            <SymbiosisIcon className="w-8 h-8 text-nexus-secondary" />
            <div>
              <h2 className="text-xl font-bold text-nexus-text-primary">Nexus Symbiosis Chat</h2>
              <p className="text-sm text-nexus-text-secondary truncate max-w-md">Topic: {context.topic}</p>
            </div>
          </div>
        ...
        <main className="flex-grow p-4 overflow-y-auto space-y-4">
          {messages.map((msg, index) => (
             <div key={index} className={`flex items-start gap-3 ${msg.sender === 'user' ? 'justify-end' : ''}`}>
               {msg.sender === 'ai' && <NexusLogo className="w-7 h-7 text-nexus-secondary flex-shrink-0 mt-1" />}
               <div className={`rounded-lg px-4 py-2 max-w-lg ${msg.sender === 'user' ? 'bg-nexus-accent/80 text-white' : 'bg-nexus-surface text-nexus-text-secondary'}`}>
                  <div className="prose prose-p:my-0 prose-strong:text-white" dangerouslySetInnerHTML={{ __html: parseMessage(msg.text) }}></div>
               </div>
             </div>
          ))}
          {isThinking && (
            <div className="flex items-start gap-3">
                <NexusLogo className="w-7 h-7 text-nexus-secondary flex-shrink-0 mt-1 animate-pulse" />
                <div className="rounded-lg px-4 py-2 bg-nexus-surface text-nexus-text-secondary flex items-center">
        ...
        <footer className="p-4 border-t border-nexus-border flex-shrink-0">
            <form onSubmit={handleSend} className="flex gap-2">
                <input
                    ...
                    disabled={isThinking}
                    className="w-full bg-nexus-bg border border-nexus-border rounded-lg p-3 text-nexus-text-primary focus:ring-2 focus:ring-nexus-accent focus:outline-none disabled:opacity-50"
                />
                <button type="submit" disabled={isThinking || !userInput.trim()} className="bg-nexus-accent hover:bg-amber-400 text-white font-bold py-2 px-6 rounded-lg transition-colors disabled:bg-gray-500 disabled:cursor-not-allowed">
                    Send
                </button>
            </form>
        </footer>
