...
export const AnalysisModal: React.FC<AnalysisModalProps> = ({ item, region, onClose }) => {
          ...
          <div id="analysis-report-content" className="report-document p-8 sm:p-12 mx-auto my-6 max-w-4xl">
              ...
              <main>
                  {parsedReport.sections.map((section, index) => (
                      <section key={index}>
                          <h2 className="report-section-title text-2xl">{section.title}</h2>
                          <div className="space-y-4">
                              {section.content.map((p, pIndex) => (
                                  <p key={pIndex} className="text-base leading-relaxed">{p}</p>
                              ))}
                          </div>
                      </section>
                  ))}
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-gradient-to-b from-nexus-surface to-nexus-bg border border-nexus-border rounded-xl shadow-2xl w-full max-w-5xl h-[90vh] flex flex-col">
        <header className="p-4 flex justify-between items-center border-b border-nexus-border flex-shrink-0">
          <div className="flex items-center gap-3">
            <NexusLogo className="w-8 h-8 text-nexus-accent" />
            <div>
              <h2 className="text-xl font-bold text-nexus-text-primary">Nexus Intelligence Deep-Dive</h2>
              <p className="text-sm text-nexus-text-secondary truncate max-w-md">Topic: {item.project_name}</p>
            </div>
          </div>
          <button onClick={onClose} className="p-1 text-nexus-text-secondary hover:text-white"><CloseIcon className="w-6 h-6"/></button>
        </header>

        <main className="flex-grow bg-nexus-bg overflow-y-auto">
            {renderContent()}
        </main>

        <footer className="p-4 flex justify-end items-center border-t border-nexus-border flex-shrink-0">
            {error && <p className="text-sm text-nexus-error mr-auto">{error}</p>}
            <button
              onClick={handleDownloadPdf}
              disabled={isLoading || isDownloading || !parsedReport}
              className="bg-nexus-accent hover:bg-amber-400 text-white font-bold py-2 px-4 rounded-lg transition-colors disabled:bg-gray-500 disabled:cursor-not-allowed flex items-center gap-2"
            >
