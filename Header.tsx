...
const NavButton: React.FC<{
    ...
    <button
        ...
        className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-colors ${
            isActive ? 'bg-nexus-surface text-nexus-accent' : 'text-nexus-text-secondary hover:text-nexus-accent hover:bg-nexus-surface/50'
        }`}
        title={title}
    >
        {children}
...
const Header: React.FC<HeaderProps> = ({ currentView, onViewChange }) => {
  return (
    <header className="p-4 md:px-8 border-b border-nexus-border sticky top-0 bg-nexus-bg/80 backdrop-blur-sm z-50 flex-shrink-0">
      <div className="container mx-auto flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
            <NexusLogo className="w-10 h-10 text-nexus-accent" />
            <div>
                <h1 className="text-xl md:text-2xl font-bold text-nexus-text-primary tracking-wide">
                BWGA Nexus AI
                </h1>
                <p className="text-xs text-nexus-text-secondary">Global Economic Empowerment OS</p>
            </div>
        </div>
